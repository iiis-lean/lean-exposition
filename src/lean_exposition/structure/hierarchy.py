...
from .order import NarrativeOrder, OrderEdge, OrderProblem, solve_order, validate_scope_order
...
def build_hierarchy(workspace, repo_key, *, config=None, source=None, source_spec=None,
                    narrative_order=None, keep_separate=(), edge_weights=None,
                    region_burden_features=None):
    hierarchy, _ = _construct(workspace, repo_key, config=config, source=source, source_spec=source_spec,
                              narrative_order=narrative_order, keep_separate=keep_separate,
                              edge_weights=edge_weights, region_burden_features=region_burden_features)
    return hierarchy


def derive_narrative_order(workspace, repo_key, *, config=None, source=None, source_spec=None,
                           keep_separate=()):
    _, order = _construct(workspace, repo_key, config=config, source=source, source_spec=source_spec,
                          keep_separate=keep_separate, order_only=True)
    return order


def _construct(workspace, repo_key, *, config=None, source=None, source_spec=None,
               narrative_order=None, keep_separate=(), edge_weights=None,
    ...
    if region_burden_features is not None:
        # Import locally: feature extraction itself depends on the structure package.
        from lean_exposition.features.core import FEATURE_CONFIG_DIGEST, digest, ref_key
        ...
        if (feature_data["workspace_digest"] != workspace.digest() or
                feature_data["config_digest"] != FEATURE_CONFIG_DIGEST or feature_data["repo_key"] != repo_key or
        ...
        burden_settings = {"policy": "strict_equal_material", "workspace_digest": feature_data["workspace_digest"],
            "feature_config_digest": feature_data["config_digest"], "feature_hierarchy_id": feature_data["hierarchy_id"],
            "feature_digest": digest(feature_data), "objective": "equal primary DP cost only; minimize sum of binary split absolute material imbalance",
    ...
    groups, owners, diagnostics = aggregate_helpers(graph, repo_key, HelperSource(), config, keep_separate)
    ...
    def collect_order_problems(current):
        ...
        if source.sequence_spec is not None:
            for sequence in source.sequence_spec.sequences:
                ...
                sequence_atoms = tuple(child for _, child in sorted(ranked, key=lambda item: (item[0], item[1])))
                if len(sequence_atoms) > 1:
                    protected.append(sequence_atoms)
        problems.append(OrderProblem(
            ...
            source_keys, source_basis, source_roles, source_anchors, tuple(protected)))
    collect_order_problems(root)

    order_config = {
        "helper": {
            "native_helper": config.native_helper,
            "lc_cross_file_helper": config.lc_cross_file_helper,
    ...
    if narrative_order is None:
        ...
        narrative_order = NarrativeOrder.create(
            ...
            diagnostics=tuple(diagnostic for scope in scope_orders for diagnostic in scope.diagnostics))
    else:
        ...
        if (narrative_order.repo_key != repo_key or narrative_order.repository_revision != repo.revision or
                ...
                narrative_order.config_digest != order_config_digest):
